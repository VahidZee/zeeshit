//
// Created by Vahid Zee on 9/22/18.
//
...
//Functions
    ...
    cv::Mat forceResizeCardAreaImage( cv::Mat& cardAreaImage ){
        ...
        cv::resize(cardAreaImage, resizedImage, cv::Size(CARD_DEFAULT_WIDTH , (int ) ( CARD_DEFAULT_WIDTH  / CARD_ASPECT_RATIO ) ), 1, 1);
    ...
    int* countLables(std::vector<int> labels ,cv::Mat data , int wLen , int hLen , int i , int  j , int sliderValue  , int level = 1 ) {
        //Local Variables
...
//                data.at<float>(x + y * wLen , 0) = 255.0;
                }
        ...
        if( j != 0 )
            for( int x = 0 ; x < wLen ; x++ )
                for( int y = 0 ; y < level ; y++ ) {
                    lableCount[ labels[ x + y * wLen ]]++;
//                data.at<float>(x + y * wLen , 2) = 255.0;
                }
        ...
        if( i != 0 )
            for( int y = 0 ; y < hLen ; y++ )
                for( int x = 1 ; x <= level ; x++ ){
                    colorCount[ ( int )( clustImage.at<cv::Vec3b>( j*hLen +  y  , i * wLen - x  )[0]  / 255.0f * (sliderValue - 1) ) ]++ ;
    //                clustImage.at<cv::Vec3b>(  j*hLen + y   , i * wLen - x  )[1] = 255;
                }
        ...
        if( j != 0 )
            for( int x =0  ; x < wLen ; x++ )
                for( int y = 1 ; y <= level ; y++ ){
                    colorCount[ ( int )( clustImage.at<cv::Vec3b>( j * hLen - y  , i * wLen + x  )[0]  / 255.0f * (sliderValue - 1) ) ]++ ;
    //                clustImage.at<cv::Vec3b>( j * hLen  , i * wLen + x  )[1] = 255;
                }
        return colorCount;
    }
    ...
    int* getColorMap( std::vector<int> labels , cv::Mat& clustImage , cv::Mat data , int wLen , int hLen , int i , int j ,  int sliderValue , int level = 1 ) {
        ...
        for( int x = 0 ; x < sliderValue ; x++ )
            std::cout<<"label:"<<lablesOnly[x]<< "->" << lablesCount[x] << "\tcolor:" << colorsOnly[x]<< "->" << colorsCount[x] << std::endl;
    ...
    int* getColorMap( cv::Mat1f colors , int sliderValue ) {
    ...
    //    for( int x = 0 ; x < sliderValue ; x++ )
    //        std::cout<<"color:"<<grayLabel[x]<< "->" << grayScaleColors[x] << std::endl;
    ...
    //    for( int x = 0 ; x < sliderValue ; x++ )
    //        std::cout<<"color:"<<colorMap[x]<< "->" << grayScaleColors[x] << std::endl;
    //    std::cout<<"\n\n+++++\n\n";
    ...
    cv::Mat kmeansParts( cv::Mat& image , int wPartsCount , int hPartsCount , int sliderValue  , int level  ) {
        ...
        for( int i = 0 ; i < wPartsCount ; i++ )
            for( int j = 0 ; j < hPartsCount ; j++ ){
                ...
                cv::Mat1f colors;
                cv::kmeans(data, sliderValue,labels,cv::TermCriteria(cv::TermCriteria::EPS+cv::TermCriteria::MAX_ITER, 10, 1.),10,cv::KMEANS_RANDOM_CENTERS,colors);
                if( level != -1 ) {
                    if( i == 0 && j == 0  ){for (unsigned int i = 0 ; i < singleLineSize ; i++ )
                        {
                    ...
                    auto colorMap = getColorMap( colors , sliderValue );
